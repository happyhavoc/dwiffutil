...
#[derive(Debug)]
enum Instruction {
    OriginalData { orig_file_offset: u32, length: u16 },
    UpdatedData { data: Vec<u8> },
    CopyOriginalFile,
}

impl Instruction {
    fn parse(data: &[u8], offset: &mut usize) -> Instruction {
        let len = u16::from_le_bytes(data[*offset..*offset+2].try_into().unwrap());
        if len == 0x8000 {
            *offset += 6;
            Instruction::CopyOriginalFile
        } else if len & 0x8000 != 0 {
            // Data comes from the original file
            let orig_file_offset = u32::from_le_bytes(data[*offset + 2..*offset + 6].try_into().unwrap());
            *offset = *offset + 6;
            Instruction::OriginalData { orig_file_offset, length: len & 0x7fff }
        } else {
            // Data is updated
            let data = data[*offset + 2..*offset + 2 + len as usize].to_vec();
            *offset = *offset + 2 + len as usize;
            Instruction::UpdatedData { data }
        }
    }

    fn len(&self) -> usize {
        match self {
            Instruction::OriginalData { length, .. } => *length as usize,
            Instruction::UpdatedData { data } => data.len(),
            Instruction::CopyOriginalFile => usize::max_value(),
        }
    }
}
...
struct Diff {
    ...
    version_low: u8,
    flags: u8,
    original_file_crc32: u32,
    updated_file_crc32: u32,
    file_size: u32,
    file_time: Datetime,
    filename: String,
    update_name: String,
...
impl Diff {
    fn parse(data: &[u8]) -> Option<(Diff, usize)> {
        if data[0..4] != *b"Diff" {
            return None;
        }
        let version_high = data[4];
        let version_low = data[5];
        let flags = data[6];

        let original_file_crc32 = u32::from_le_bytes(data[8..0xc].try_into().unwrap());
        let updated_file_crc32 = u32::from_le_bytes(data[0xc..0x10].try_into().unwrap());

        let file_size = u32::from_le_bytes(data[0x14..0x18].try_into().unwrap());

        let year = u16::from_le_bytes(data[0x18..0x1a].try_into().unwrap());
        let month = data[0x1a];
        let day = data[0x1b];
        let hour = data[0x1c];
        let minute = data[0x1d];
        let second = data[0x1e];

        let filename_end = memchr::memchr(0, &data[0x1f..]).unwrap() + 0x1f;
        let (filename, ..) = encoding_rs::SHIFT_JIS.decode(&data[0x1f..filename_end]);
        let filename = filename.to_string();

        let update_name_end = memchr::memchr(0, &data[filename_end+1..]).unwrap() + filename_end + 1;
        let (update_name, ..) = encoding_rs::SHIFT_JIS.decode(&data[filename_end+1..update_name_end]);
        let update_name = update_name.to_string();

        let folder_end= memchr::memchr(0, &data[update_name_end+1..]).unwrap() + update_name_end + 1;
        let (folder, ..) = encoding_rs::SHIFT_JIS.decode(&data[update_name_end+1..folder_end]);
        let folder = folder.to_string();

        let mut instructions = Vec::new();
        let mut file_offset = 0;
        let mut diff_offset = folder_end + 1;
        while file_offset < file_size {
            let insn = Instruction::parse(data, &mut diff_offset);
            file_offset = file_offset.saturating_add(insn.len() as u32);
...
fn crc32(data: &[u8]) -> u32 {
    let mut crc = 0u32;
    for &byte in data {
        crc = CRC32_TABLE[(byte as usize ^ crc as usize) & 0xff] ^ crc >> 8
    }
    crc
}

/// List the diffs in an update file.
#[derive(FromArgs, PartialEq, Debug)]
#[argh(subcommand, name = "list-diffs")]
pub struct ListDiffsSubcommand {
    #[argh(positional)]
    update_file: PathBuf,
}
...
/// Apply an update.
#[derive(FromArgs, PartialEq, Debug)]
#[argh(subcommand, name = "apply-diffs")]
pub struct ApplyDiffsSubcommand {
    #[argh(positional)]
    update_file: PathBuf,
    #[argh(positional)]
...
/// Calculates the CRC32 of the given file.
#[derive(FromArgs, PartialEq, Debug)]
#[argh(subcommand, name = "crc32")]
pub struct CheckCrc32Subcommand {
    #[argh(positional)]
    file: PathBuf,
}
...
#[argh(subcommand)]
enum Subcommand {
    List(ListDiffsSubcommand),
    Apply(ApplyDiffsSubcommand),
    CheckCrc32(CheckCrc32Subcommand),
}

/// Manipulate DWiff update files.
#[derive(FromArgs, PartialEq, Debug)]
struct Args {
    #[argh(subcommand)]
...
fn get_diffs(path: &Path) -> Vec<Diff> {
    let exe = std::fs::read(path).unwrap();
    let mut diffs = Vec::new();
    let mut diff_offset = memchr::memmem::find(&exe, b"Diff").unwrap();
    loop {
        let (diff, diff_end) = Diff::parse(&exe[diff_offset..]).unwrap();
        diffs.push(diff);
        match memchr::memmem::find(&exe[diff_offset + diff_end..], b"Diff") {
            Some(v) => diff_offset = diff_offset + diff_end + v,
            None => break,
        }
    }
    diffs
...
fn apply_diff(instructions: &[Instruction], original_path: &Path) -> NamedTempFile {
    let mut tempfile = NamedTempFile::new().unwrap();

    let mut original_file = std::fs::File::open(original_path).unwrap();
    let mut buffer = vec![0; 0x8000];
    for instruction in instructions {
        match instruction {
            Instruction::OriginalData { orig_file_offset, length } => {
                original_file.seek(SeekFrom::Start(*orig_file_offset as u64)).unwrap();
                original_file.read_exact(&mut buffer[..*length as usize]).unwrap();
                tempfile.write_all(&buffer[..*length as usize]).unwrap();
            },
            Instruction::UpdatedData { data } => {
                tempfile.write_all(&data).unwrap();
            },
            Instruction::CopyOriginalFile => {
                std::io::copy(&mut original_file, &mut tempfile).unwrap();
            },
        }
    }
...
fn apply_diffs_command(args: ApplyDiffsSubcommand) {
    let diffs = get_diffs(&args.update_file);

    for diff in diffs {
        let path = diff.path();
        let installed_file = args.installation_dir.join(&path);
        let orig_file_crc32 = crc32(&std::fs::read(&installed_file).unwrap());
        if orig_file_crc32 != diff.original_file_crc32 {
            println!("SKIP     - {} {} (expected {:#010x}, got {:#010x})", diff.update_name, path.display(), diff.original_file_crc32, orig_file_crc32);
            continue;
        }
        let tmp_path = apply_diff(&diff.instructions, &installed_file);
        let updated_file_crc32 = crc32(&std::fs::read(tmp_path.path()).unwrap());
        if updated_file_crc32 != diff.updated_file_crc32 {
...
fn list_diffs_command(args: ListDiffsSubcommand) {
    let diffs = get_diffs(&args.update_file);
...
fn check_crc32_command(args: CheckCrc32Subcommand) {
    let data = std::fs::read(&args.file).unwrap();
    println!("{} - {:#010x}", args.file.display(), crc32(&data));
}
...
fn main() {
    ...
    match args.subcommand {
        Subcommand::List(v) => list_diffs_command(v),
        Subcommand::Apply(v) => apply_diffs_command(v),
        Subcommand::CheckCrc32(v) => check_crc32_command(v),
    }
}
